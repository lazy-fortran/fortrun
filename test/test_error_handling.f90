...
contains
  ...
  subroutine test_unknown_module_error()
    ...
    test_dir = '/tmp/test_unknown_module'
    ...
    call check_exit_code('/tmp/unknown_exit.txt', 1)
    
    ! Check that output contains the FPM error message
    call check_output_contains('/tmp/unknown_output.txt', 'Unable to find source for module dependency')
    call check_output_contains('/tmp/unknown_output.txt', 'nonexistent_module')
    
    ! Clean up
    call execute_command_line('rm -rf ' // trim(test_dir))
    call execute_command_line('rm -f /tmp/unknown_output.txt /tmp/unknown_exit.txt')
  ...
  subroutine test_similar_module_suggestions()
    ...
    ! Create test directory
    test_dir = '/tmp/test_module_error'
    call execute_command_line('mkdir -p ' // trim(test_dir))
    ...
    ! Check that it failed with non-zero exit code
    call check_exit_code('/tmp/error_exit.txt', 1)
    
    ! Check that output contains the FPM error message
    call check_output_contains('/tmp/error_output.txt', 'some_missing_module')
    
    ! Clean up
    call execute_command_line('rm -rf ' // trim(test_dir))
    call execute_command_line('rm -f /tmp/error_output.txt /tmp/error_exit.txt')
  ...
  subroutine test_fpm_error_forwarding()
    ...
    ! Check that it failed with non-zero exit code
    call check_exit_code('/tmp/syntax_exit.txt', 1)
    
    ! Check that output contains error information
    call check_output_contains('/tmp/syntax_output.txt', 'Error')
    
    ! Clean up
    call execute_command_line('rm -rf ' // trim(test_dir))
    call execute_command_line('rm -f /tmp/syntax_output.txt /tmp/syntax_exit.txt')
