...
contains
    ...
    function test_preprocessor_paths() result(passed)
        logical :: passed
        character(len=:), allocatable :: f_file, preprocessed_file
        integer :: exit_code, unit
        character(len=256) :: custom_cache_dir, custom_config_dir

        print *, "Test: Preprocessor paths (.f file processing)"
        passed = .true.

        ! Create a .f file to trigger preprocessor path
        f_file = temp_mgr%get_file_path('preprocess_test.f')
        open(newunit=unit, file=f_file, status='replace')
        write(unit, '(a)') '! Test .f file preprocessing'
        write(unit, '(a)') 'x = 42'
        ...
        preprocessed_file = trim(f_file) // '.preprocessed.f90'
        ...
        call sys_remove_dir(custom_cache_dir)
