...
contains
    ...
    subroutine check_help_output(output_file, passed)
        ...
        do
            ...
            if (index(line, 'Usage: fortrun') > 0) then
                found_usage = .true.
                exit
            end if
        end do
 ...
 write (error_unit, *) 'Error: Warm cache verbose output not found (expected Cache hit)'
  ...
  write (error_unit, *) 'Error: Expected exit code ', expected, ' but got ', actual_exit
    ...
    subroutine test_f_file_execution()
        ...
        print *, 'Test 13: .lf file execution'

        call temp_mgr%create('test_f_file_exec')
        output_file = temp_mgr%get_file_path('cli_test_output.txt')
        exit_file = temp_mgr%get_file_path('cli_test_exit.txt')

        ! Create a simple .lf test file
        test_f_file = temp_mgr%get_file_path('test_simple.lf')
        open (newunit=unit, file=test_f_file, status='replace')
        write (unit, '(a)') "print *, 'Hello from .lf file'"
        close (unit)
        ...
        call check_program_output(output_file, 'Hello from .lf file', success)

        if (exit_ok .and. success) then
            print *, 'PASS: .lf file executed successfully'
        else
            write (error_unit, *) 'FAIL: .lf file execution failed'
            stop 1
        end if
    ...
    command = fortran_with_isolated_cache('invalid_ext')//' "'//trim(test_txt_file)//'"'
        ...
        call check_program_output(output_file, &
                               'must have .f90, .F90, .lf, or .LF extension', has_error)
    ...
    subroutine test_flag_option()
        ...
        ! Test with .lf file (should combine with opinionated flags)
        print *, '  Testing .lf file with --flag option'

        ! Create a test .lf file
        test_flag_file = temp_mgr%get_file_path('test_flag_option.lf')
        open (newunit=unit, file=test_flag_file, status='replace')
        write (unit, '(a)') 'print *, "Flag test output with .lf file"'
        close (unit)
      ...
      command = fortran_with_isolated_cache('flag_f') // ' --flag "-Wall" "'//trim(test_flag_file)//'"'
        ...
        call check_exit_code(exit_file, 0, exit_ok)
    call check_program_output(output_file, 'Flag test output with .lf file', has_output)

        if (exit_ok .and. has_output) then
            print *, 'PASS: --flag option works with .lf files'
        else
            write (error_unit, *) 'FAIL: --flag option failed with .lf files'
            stop 1
        end if
