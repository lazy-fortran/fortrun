...
contains
    ...
    subroutine test_global_scope()
        ...
        value = create_literal(LITERAL_INTEGER, "42", 1, 12)
    ...
    subroutine test_function_scope()
        ...
        value = create_literal(LITERAL_INTEGER, "10", 2, 9)
    ...
    subroutine test_nested_block_scopes()
        ...
        one = create_literal(LITERAL_INTEGER, "1", 1, 9)
        ...
        zero = create_literal(LITERAL_INTEGER, "0", 2, 9)
        condition = create_binary_op(x_ref, zero, ">", 2, 7)

        ! Then block: y = 1
        y_ref = create_identifier("y", 3, 5)
        one = create_literal(LITERAL_INTEGER, "1", 3, 9)
        ...
        two = create_literal(LITERAL_INTEGER, "2", 4, 9)
        else_assign = create_assignment(y_ref, two, 4, 5)
        allocate (else_wrapper%node, source=else_assign)

        ! Create if statement
        if_stmt = create_if(condition, [then_wrapper], [else_wrapper], 2, 1)

        ! Analyze if statement
        typ = ctx%analyze_if_node(if_stmt)
    ...
    subroutine test_module_scope()
        ...
        decl = create_declaration("real", "module_var", 1, 1)
        allocate (decl_wrapper%node, source=decl)

        mod = create_module("test_module", [decl_wrapper], 1, 1)

        ! Analyze module
        typ = ctx%analyze_module(mod)
    ...
    subroutine test_scope_shadowing()
        ...
        real_val = create_literal(LITERAL_REAL, "3.14", 3, 9)
    ...
    subroutine test_loop_variable_scope()
        ...
        ! Create do loop
        start_val = create_literal(LITERAL_INTEGER, "1", 1, 10)
        end_val = create_literal(LITERAL_INTEGER, "10", 1, 15)
        ...
        loop = create_do_loop("i", start_val, end_val, [body_wrapper], 1, 1)

        ! Analyze loop
        typ = ctx%analyze_do_loop(loop)
