...
contains
    ...
    function push_program(stack, name, body_indices, line, column) result(prog_index)
    ...
    function push_assignment(stack, target_index, value_index, line, column, parent_index) result(assign_index)
        type(ast_stack_t), intent(inout) :: stack
    ...
    function push_binary_op(stack, left_index, right_index, operator, line, column, parent_index) result(binop_index)
        type(ast_stack_t), intent(inout) :: stack
    ...
    function push_function_call(stack, name, arg_indices, line, column, parent_index) result(call_index)
        type(ast_stack_t), intent(inout) :: stack
        character(len=*), intent(in) :: name
    ...
    function push_identifier(stack, name, line, column, parent_index) result(id_index)
        type(ast_stack_t), intent(inout) :: stack
        character(len=*), intent(in) :: name
        integer, intent(in), optional :: line, column, parent_index
        integer :: id_index
 ...
 function push_literal(stack, value, kind, line, column, parent_index) result(lit_index)
        type(ast_stack_t), intent(inout) :: stack
        ...
        do i = 1, size(node_specs)
            block
                ...
                if (colon_pos > 0) then
                    ...
                    case ('identifier')
                        indices(i) = push_identifier(stack, node_name, i, 1)
                    case ('literal_int')
                      indices(i) = push_literal(stack, node_name, LITERAL_INTEGER, i, 1)
                    case ('literal_real')
                        indices(i) = push_literal(stack, node_name, LITERAL_REAL, i, 1)
                    case ('literal_string')
                       indices(i) = push_literal(stack, node_name, LITERAL_STRING, i, 1)
                    case default
                        indices(i) = push_identifier(stack, node_name, i, 1)
                    end select
                else
                    indices(i) = push_identifier(stack, spec, i, 1)
                end if
            end block
        end do
