! Exact replica of test_cache_lock.f90 but as an app instead of a test
program test_cache_lock_as_app
    use cache_lock
    ...
    print '(a)', 'test_cache_lock_as_app: Initialization complete'
    flush (6)

    print '(a)', 'test_cache_lock_as_app: About to call create_test_cache_dir'
    flush (6)

    ! Create temporary directory for testing with unique suffix
    print '(a)', 'test_cache_lock_as_app: Creating temp directory...'
    flush (6)

    ! Add debug print before calling create_test_cache_dir
    print '(a)', 'test_cache_lock_as_app: DEBUG - About to call create_test_cache_dir'
    flush (6)

    temp_cache_dir = create_test_cache_dir('cache_lock_test')

    ! Add debug print after successful call
    print '(a)', 'test_cache_lock_as_app: DEBUG - create_test_cache_dir call completed'
    flush (6)

    print '(a,a)', 'test_cache_lock_as_app: Created directory: ', trim(temp_cache_dir)
    flush (6)
    ...
    block
        logical :: dir_exists
       print '(a)', 'test_cache_lock_as_app: DEBUG - About to check if directory exists'
        flush (6)

        inquire (file=trim(temp_cache_dir), exist=dir_exists)

        print '(a,l)', 'test_cache_lock_as_app: DEBUG - Directory exists check result: ', dir_exists
        flush (6)
    ...
    print '(a)', 'test_cache_lock_as_app: DEBUG - About to call acquire_lock'
    flush (6)

    success = acquire_lock(trim(temp_cache_dir), 'test_project', .false.)

    print '(a)', 'test_cache_lock_as_app: DEBUG - acquire_lock call completed'
    flush (6)
...
end program test_cache_lock_as_app
